...
<template>
  <section id="skills" class="py-16 md:py-24 max-w-5xl mx-auto w-full flex flex-col gap-10">
    ...
    <div v-reveal:base class="flex flex-col md:flex-row md:items-end justify-between gap-8 pb-4">
    ...
    <div v-reveal:skills class="flex flex-col gap-6">
      ...
      >
        <button
          ...
          :class="[
            'px-4 sm:px-5 py-2 rounded-full font-display text-xs sm:text-sm font-semibold transition-all duration-300 ease-out hover:scale-105 active:scale-95 whitespace-nowrap cursor-pointer',
            selectedCategory === category.id
        ...
        <Transition name="skills-view" mode="out-in">
          <div :key="selectedCategory" class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
            <div
              ...
              :key="skill.name"
              class="skill-card-item group p-4 rounded-xl bg-surface-container-low/80 hover:bg-surface-container shadow-sm border border-black/[0.04] dark:border-white/[0.06] flex items-center gap-4 cursor-default select-none"
              :style="{
                '--stagger': idx,
              }"
            >
              <div
                class="w-10 h-10 rounded-lg bg-surface-container-high flex items-center justify-center text-primary group-hover:bg-primary-container group-hover:text-white transition-all duration-400 ease-out shrink-0 shadow-inner group-hover:scale-110 group-hover:rotate-6 group-hover:shadow-[0_0_16px_rgba(45,104,255,0.45)]"
              >
                <span
                  class="material-symbols-outlined text-[20px] transition-transform duration-400 ease-out"
                  >{{ skill.iconName }}</span
                >
              </div>
              <div class="flex flex-col min-w-0">
                <span
                  class="font-display text-sm sm:text-base text-on-surface group-hover:text-primary transition-colors duration-300 font-medium truncate"
                >
                  {{ skill.name }}
                </span>
                <span
                  class="font-mono text-[10px] text-outline uppercase tracking-wider transition-colors duration-300 group-hover:text-on-surface-variant"
                >
                  {{ skill.category }}
                </span>
              </div>
